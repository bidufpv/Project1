...
import { getBrand } from "@/lib/firestore/brands/read-server";
...
//function for handle update the Brand
  const handleUpdate =async()=>{
    ...
    try {
      ...
      setData(null); // Reset form data
      setImage(null); // Reset image state
      router.push("/admin/brands"); // Redirect to the brands page
    } catch (error) {
      toast.error(error?.message);
  ...
  return (
    <div className="bg-slate-100 p-5 flex flex-col gap-5 rounded-xl w-full md:w-[400px]">
      ...
      {/* Form title */}
      <h1 className="font-semibold">{id ?'Update':'Create'} a Brand</h1>
