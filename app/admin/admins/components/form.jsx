...
import { getAdmin } from "@/lib/firestore/admins/read-server";
...
//function for handle update the Admin
  const handleUpdate =async()=>{
    ...
    try {
      ...
      setData(null); // Reset form data
      setImage(null); // Reset image state
      router.push("/admin/admins"); // Redirect to the admins page
    } catch (error) {
      toast.error(error?.message);
  ...
  return (
    <div className="bg-slate-100 p-5 flex flex-col gap-5 rounded-xl w-full md:w-[400px]">
      ...
      {/* Form title */}
      <h1 className="font-semibold">{id ?'Update':'Create'} a Admin</h1>
  ...
  >
    {isloading ? (
      ...
      (id ? "Update" : "Create") // Conditional button text based on form mode
    )}
  </button>
</div>
